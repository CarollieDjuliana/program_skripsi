...
class RandomForest:
    ...
    def _print_tree(self, node, depth=0):
        if 'predicted_class' in node:
            print(f"{'  ' * depth}Predicted Class: {node['predicted_class']}")
            return

        print(
            f"{'  ' * depth}Feature: {node['feature']}, Threshold: {node['threshold']}")
        print(f"{'  ' * depth}Left:")
...
class DecisionTreeVisualization:
    ...
    def visualize_tree(self, forest):
        for i, tree in enumerate(forest.trees):
            self.ax.set_title(f"Tree {i+1}")
            self.depth = 0
            self.pos = {}
            self._visualize_tree(tree.tree)

    def _visualize_tree(self, node, depth=0, parent=None):
        if parent is None:
            pos = (0.5, 1)
        else:
            parent_pos = self.pos[parent]
            pos = (parent_pos[0], parent_pos[1] - 0.1)

        if 'predicted_class' in node:
            self.ax.annotate(f"Predicted Class: {node['predicted_class']}",
                             xy=pos, xycoords='axes fraction',
                             xytext=(pos[0], pos[1] - 0.1), textcoords='axes fraction',
                             arrowprops=dict(arrowstyle="->",
                                             connectionstyle="arc3"))
        else:
            self.ax.annotate(f"Feature: {node['feature']}\nThreshold: {node['threshold']}",
                             xy=pos, xycoords='axes fraction',
                             xytext=(pos[0], pos[1] - 0.1), textcoords='axes fraction',
                             arrowprops=dict(arrowstyle="->",
                                             connectionstyle="arc3"))

            left_pos = (pos[0] - 0.05, pos[1] - 0.1)
            right_pos = (pos[0] + 0.05, pos[1] - 0.1)

            self.ax.plot([pos[0], left_pos[0]], [
                         pos[1], left_pos[1]], linestyle='dotted')
            self.ax.plot([pos[0], right_pos[0]], [
                         pos[1], right_pos[1]], linestyle='dotted')

            self._visualize_tree(node['left'], depth + 1, parent=node)
            self._visualize_tree(node['right'], depth + 1, parent=node)
...
model = RandomForest(n_trees=3, max_depth=3)

# Latih model dengan data latih
model.fit(X_train, y_train)
# tree_visualizer = DecisionTreeVisualization()
# tree_visualizer.visualize_tree(model)
